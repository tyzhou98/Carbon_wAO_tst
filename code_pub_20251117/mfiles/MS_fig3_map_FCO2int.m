% create year-to-year map of carbon uptake
%
%                  Tianyu Zhou, UDel, 11/07/2025
%                       Yun Li, UDel, 11/12/2025

clc; clear; close all; info_params
%========================================================================
% Edit the following based on user's needs
isfig = 1; ffig = [fdir_MSfig 'fig3_map_UCO2'];
fdiag = [fdir_MSdata num2str(RID,'R%i_') fRF_cdiag];
%========================================================================
%###################
%## plot settings ##
%###################
...
ccut = 0.1; clims_num = [0 ccut*2];    % the scaled clims  ?????????
...
%###############
load(fdiag)
FCO2int = diag.i2A.FCO2int.*cff_g2Gg;
FCO2int(diag.i2A.nbins==0|diag.icet>900) = NaN;
Pow = diag.i2A.Pow;
...
for kp = 1:length(obs_yrs); yrnow = obs_yrs(kp); yid = find(diag.years==yrnow);
  ...
  [cmap_adj,clims_adj,wrk] = fun_adj_pcolor(cmap,clims_num,wrk,color_land,color_ice);
  ...
  [C,h]=contour(px,py,wrk,[30 60 90],'color','w','linewidth',0.5);
  clabel(C,h,'color','w','fontsize',fsize-5,'fontweight','bold');
...
title(hco,{['\boldmath' label_UCO2_GgC(1:25) '$'],...
           ['\boldmath$' label_UCO2_GgC(26:end)]},'fontsize',fsize,'interpreter','latex')
set(hco,'xlim',clims_num,'xtick',cticks,'xticklabel',cticklabels)
% broken axis
