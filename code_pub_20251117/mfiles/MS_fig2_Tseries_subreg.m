...
%   - monthly pCO2 obs vs mod for subregions
%   - yearly  pCO2, carbon uptake, open water area 
%
%             Tianyu Zhou, UDel, 10/12/2025

clc; clear; close all; info_params
%=========================================================================
% Edit the following based on user's purpose
mons = [4 8];  % data months on the plot
favg = [fdir_MSdata 'Tseries_regavg_6kmOUYgrd.mat'];
...
vinfo = {'dpCO2avg','dlims','dticks',label_dpCO2   ,   [0 0 0] , 0.05 ;...
	 'Aow'     ,'alims','aticks',label_Aow     , color_Aow , 0.85 ;...
	 'FCO2int' ,'flims','fticks',label_UCO2_TgC, color_UCO2, 1.10};
...
for kr = 1:3 % three subregions
  ...
  if kr==1; text(xlims(1),plims(2)+0.05*diff(plims),['\boldmath' label_pCO2],...  % variable label
  ...
  for kv = 1:3;
    ...
    ylabels = ['\boldmath' vinfo{kv,4}];                % y-axis labels
    ...
    if strcmp(vinfo{kv,1},'FCO2int')                    % bar plot for Fint
      hb = bar(ty2y,wrk,0.7);
      set(hb,'FaceColor', colors,'Edgecolor','none','facealpha',0.5,'showbaseline','off')
    ...
    if kr==1; text(tlims(1)+diff(tlims)*vinfo{kv,6},ylims(2)+0.05*diff(ylims),ylabels,...
...
%## save figure ##
%#################
if isfig; print('-dpng','-r400',ffig); end
