% The figure includes the following panels:
%   - data number + RF model eval for each year
%   - map of pCO2 obs
%   - 1:1 plot for training
%   - 1:1 plot for testing
%
%             Tianyu Zhou, UDel, 09/01/2025
%                  Yun Li, UDel, 10/18/2025

clc; clear; close all; info_params
%=====================================================================
% edit the following based on the user's needs
feva = [fdir_MSdata num2str(RID,'R%.3i_') fRF_output_eva]; % ML evaluation
fOUY = [fdir_MSdata 'Tseries_regavg_6kmOUYgrd.mat'];       % OUY subregions
isfig = 1; ffig = [fdir_MSfig 'fig1_obs_map_1to1_'];
%=====================================================================
%#####################
%## figure settings ##
%#####################
...
cff = 0.1;                       % streching cff for the additional x-axis
% obs map
fgxm =-0.09; fgwm = 0.73;
fgym = 0.29; fghm = fgwm;
pclims = [100 450]; pcticks = 100:50:500;  % pCO2
load([fdir_MSdata 'cmap_bathy5.mat']); bcmap = cmap;
load([fdir_MSdata 'cmap_pCO2.mat']);   pcmap = cmap;
% 1:1 plots
fgxp = 0.60; fgwp = 0.35; fgdh2 = fgwp+0.03;
fgyp = 0.50; fghp = fgwp;
plims = [50 650];                % lims of the 1:1 plot
load([fdir_MSdata 'cmap_pCO2dens.mat'])
...
wrk(wrk>=-3500 & wrk<-2500) = -180; mgrid.Bathy = wrk;  % remap bathymetry
bclims = [-180 0]; bcticks= -180:20:0; bcticklabels = num2str([-3500:1000:-500 -100:20:0]','%i'); % bathy labels
% Ouyang Mask
...
pcolor( px,py,mgrid.Bathy); shading flat         % color the land
contour(px,py,mgrid.mask,[1 1]*0.99,'color','k') % coastal lines
colormap(ax,cmap_adj); caxis(clims_adj)          % adjusted colorbar and colormap for land
...
title(hco,label_pCO2,'fontsize',fsize-1,'interpreter','latex')
set(hco,'xlim',pclims,'xtick',pcticks,'fontsize',fsize-1)
hco.Ruler.TickLabelRotation = 0;
% OUY subregion boundaries
for kr=1:3
  contour(px,py,OUY.mask,[1 1]*kr,'color',[1 1 1]*0.15,'linewidth',0.5)
end
...
for kK = 1:2
  ...
  plot(x+1000,wrk(:,2),':' ,'linewidth',1,'color','k')
...
text(plims(1)-0.21*diff(plims),plims(2)+0.01*diff(plims),label_pCO2,'Interpreter','latex','fontsize',fsize+1,'rot',90)
text(plims(1)+0.08*diff(plims),plims(1)-0.15*diff(plims),'Observation','fontsize',fsize+1)
text(plims(1)+0.54*diff(plims),plims(1)-0.16*diff(plims),label_pCO2,'Interpreter','latex','fontsize',fsize+1)
